...
function fitGarchModelWithParams(
  ...
  if (modelType === 'garch') {
    ...
    if (initialVar <= 0 || !isFinite(initialVar)) {
      console.error('[GARCH-DEBUG] EGARCH: Invalid initial variance:', initialVar);
    ...
    for (let i = 1; i < returnsPct.length; i++) {
      ...
      const absEpsilonPrev = Math.abs(epsilonPrev);
      lnSigma2 = omega + alpha * epsilonPrev + gamma * (absEpsilonPrev - Math.sqrt(2 / Math.PI)) + beta * lnSigma2_clamped;
      ...
      if (!isFinite(sigma2) || sigma2 <= 0) {
        console.error('[GARCH-DEBUG] EGARCH: Invalid sigma2 at iteration', i, 'lnSigma2:', lnSigma2, 'sigma2:', sigma2);
        sigma2 = Math.max(sigma2, 1e-8);
      }
    ...
    if (!isFinite(sigma2) || sigma2 <= 0) {
      ...
      console.error('[GARCH-DEBUG] EGARCH: Invalid finalVariance:', sigma2, 'using clamped value:', finalVar);
      sigma2 = Math.max(finalVar, 1e-8);
    }
    
    return {
      type: 'egarch',
      params: { omega, alpha, gamma, beta },
      finalVariance: Math.max(sigma2, 1e-8),
      lastReturn: returnsPct[returnsPct.length - 1],
      conditionalVolatility: conditionalVol,
    };
  }
